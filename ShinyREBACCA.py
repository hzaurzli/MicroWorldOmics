...
# Form implementation generated from reading ui file 'ShinyREBACCA.ui'
#
...
class winTest(QtWidgets.QWidget):
    ...
    def closeEvent(self, event):
        ...
        """
        try:
            with os.popen('netstat -aon|findstr "50737"') as res:
                res = res.read().split('\n')
            result = []
            for line in res:
            ...
            if len(result) != 0:
                for task in result:
                    if task['address'] == '127.0.0.1:50737':
                        pid = task['pid']
...
class WorkThread(QThread):
    ...
    def run(self):
        ...
        os.popen(path + '/Shiny/R-4.3.2/bin/Rscript.exe ' +
                 path + '/Shiny/ShinyScript/ShinyREBACCA/ShinyREBACCA.R')

        while True:
            try:
                http = urllib3.PoolManager()
                http.request('GET', 'http://127.0.0.1:50737')

                self.trigger.emit('ShinyApp has been started!!!')
            except:
                time.sleep(5)

class ShinyREBACCA_Form(QWidget):
    ...
    def retranslateUi(self, Form):
        ...
        Form.setWindowTitle(_translate("Form", "ShinyREBACCA"))
        self.label.setText(_translate("Form", "ShinyREBACCA"))
    ...
    def open(self):
        ...
        if textbrowser == 'ShinyApp has been started!!!':
            self.winTable = ShinyWeb_Form(port=50737)
            self.winTable.show()
        else:
            w = QWidget()
