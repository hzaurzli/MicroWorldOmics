...
# Form implementation generated from reading ui file 'ShinySpiecEasi.ui'
#
...
class winTest(QtWidgets.QWidget):
    ...
    def closeEvent(self, event):
        ...
        """
        try:
            with os.popen('netstat -aon|findstr "50736"') as res:
                res = res.read().split('\n')
            result = []
            for line in res:
            ...
            if len(result) != 0:
                for task in result:
                    if task['address'] == '127.0.0.1:50736':
                        pid = task['pid']
...
class WorkThread(QThread):
    ...
    def run(self):
        ...
        os.popen(path + '/Shiny/R-4.3.2/bin/Rscript.exe ' +
                 path + '/Shiny/ShinyScript/ShinySpiecEasi/ShinySpiecEasi.R')

        while True:
            try:
                http = urllib3.PoolManager()
                http.request('GET', 'http://127.0.0.1:50736')

                self.trigger.emit('ShinyApp has been started!!!')
            except:
                time.sleep(5)

class ShinySpiecEasi_Form(QWidget):
    ...
    def retranslateUi(self, Form):
        ...
        Form.setWindowTitle(_translate("Form", "ShinySpiecEasi"))
        self.label.setText(_translate("Form", "ShinySpiecEasi"))
    ...
    def open(self):
        ...
        if textbrowser == 'ShinyApp has been started!!!':
            self.winTable = ShinyWeb_Form(port=50736)
            self.winTable.show()
        else:
            w = QWidget()
